...
function createMockDiscoveryService(instances: object[]): Partial<DiscoveryService> {
  return {
    getProviders: () =>
...
describe('SagaRunnerProvider', () => {
  ...
  let runner: { start: ReturnType<typeof vi.fn>; stop: ReturnType<typeof vi.fn> };
  ...
  it('should skip providers without @SagaParticipant metadata', async () => {
    ...
    const discovery = createMockDiscoveryService([regularInstance, participant]);
    const provider = new SagaRunnerProvider(
      discovery as DiscoveryService,
      registry,
      runner as unknown as SagaRunner,
