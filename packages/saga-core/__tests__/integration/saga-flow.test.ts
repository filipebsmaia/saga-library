...
describe('Integration: Saga Flow', () => {
  ...
  describe('Happy path: order → payment → inventory', () => {
    it('should propagate events through the saga chain', async () => {
      const paymentHandler = vi.fn().mockImplementation(
        async (event: IncomingEvent, emit: Emit) => {
          await emit({
      ...
      const inventoryHandler = vi.fn().mockImplementation(
        async (event: IncomingEvent, emit: Emit) => {
          await emit({
      ...
      await emit({ eventType: 'order.created', stepName: 'order', payload: { orderId: 'ORD-123' } });
  ...
  describe('Compensation flow', () => {
    it('should handle non-retryable errors and allow compensation', async () => {
      ...
      const paymentHandler = vi.fn().mockImplementation(
        async (_event: IncomingEvent, emit: Emit) => {
          await emit({ eventType: 'payment.processed', stepName: 'payment', payload: { status: 'ok' } });
        },
      );
      ...
      const compensationHandler = vi.fn().mockImplementation(
        async (_event: IncomingEvent, emit: Emit) => {
          await emit({ eventType: 'payment.refunded', stepName: 'compensation', payload: { reason: 'inventory_failed' } });
        },
      );
      ...
      await emit({ eventType: 'order.created', stepName: 'order', payload: { orderId: 'ORD-456' } });
      ...
      await emit({ eventType: 'payment.compensate', stepName: 'compensation', payload: { reason: 'inventory_failed' } });
  ...
  describe('Retry exhausted', () => {
    it('should retry and call onRetryExhausted when all retries fail', async () => {
      ...
      const handler = vi.fn().mockRejectedValue(new SagaRetryableError('service unavailable'));
      const onRetryExhausted = vi.fn().mockImplementation(
        async (_event: IncomingEvent, _error: SagaRetryableError, emit: Emit) => {
          await emit({ eventType: 'order.failed', stepName: 'order', payload: { reason: 'retries_exhausted' } });
        },
      );
      ...
      const emitPromise = emit({ eventType: 'order.created', stepName: 'order', payload: { orderId: 'ORD-789' } });
