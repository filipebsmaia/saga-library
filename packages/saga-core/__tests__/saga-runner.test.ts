...
import type { SagaTransport, InboundMessage } from '../src/transport/transport.interface';
...
function makeInboundMessage(eventType: string, sagaId = 'saga-123'): InboundMessage {
  return {
    topic: eventType,
    key: sagaId,
...
describe('SagaRunner', () => {
  ...
  describe('retry with backoff', () => {
    it('should retry on SagaRetryableError with exponential backoff', async () => {
      ...
      const handlePromise = subscribeHandler!(makeInboundMessage('order.created'));
    ...
    it('should call onRetryExhausted when retries are exhausted', async () => {
      const handler = vi.fn().mockRejectedValue(new SagaRetryableError('always fails'));
