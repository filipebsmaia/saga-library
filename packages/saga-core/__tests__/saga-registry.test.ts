...
function makeParticipant(serviceId: string, events: Record<string, () => Promise<void>>): SagaParticipant {
...
describe('SagaRegistry', () => {
  ...
  it('should handle participant with multiple event types', () => {
    ...
    const handlerA = async () => {};
    const handlerB = async () => {};
    registry.register(makeParticipant('svc-a', { 'event.a': handlerA, 'event.b': handlerB }));
