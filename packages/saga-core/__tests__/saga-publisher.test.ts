...
import type { SagaTransport, OutboundMessage } from '../src/transport/transport.interface';
import type { OtelContext } from '../src/otel/otel-context';

function createMockTransport(): SagaTransport & { publishedMessages: OutboundMessage[] } {
...
function createMockOtel(): OtelContext & {
  ...
  return {
    ...
    enrichSpan: vi.fn(),
    withSpan: vi.fn().mockImplementation(async (_name: string, _attrs: Record<string, string>, fn: () => Promise<any>) => fn()),
    injectTraceContext: vi.fn(),
    withExtractedSpan: vi.fn().mockImplementation(async (_name: string, _attrs: Record<string, string>, _headers: Record<string, string>, fn: () => Promise<any>) => fn()),
  };
}
...
describe('SagaPublisher', () => {
  ...
  describe('forSaga (genesis — no parent context)', () => {
    it('should return an Emit that publishes with rootSagaId = sagaId', async () => {
      ...
      await emit({ eventType: 'order.created', stepName: 'order', payload: { orderId: '456' } });
  ...
  describe('forSaga (sub-saga — with parent context)', () => {
    it('should propagate parentSagaId and rootSagaId', async () => {
      ...
      await emit({ eventType: 'payment.processed', stepName: 'payment', payload: { amount: 100 } });
  ...
  describe('publish', () => {
    it('should call otelCtx.injectBaggage and enrichSpan', async () => {
      ...
      expect(otel.injectBaggage).toHaveBeenCalledWith('saga-123', 'saga-123', undefined);
