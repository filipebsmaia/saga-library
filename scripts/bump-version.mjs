...
if (!version) {
  const current = JSON.parse(readFileSync(resolve(rootDir, packages[0]), 'utf-8')).version;
...
function parseVersion(versionString) {
  const preMatch = versionString.match(/^(\d+)\.(\d+)\.(\d+)(?:-([a-zA-Z]+)\.(\d+))?$/);
  if (!preMatch) {
    return null;
  }
  return {
...
function bumpVersion(current, bumpType, preIdentifier) {
  ...
  switch (bumpType) {
    case 'major':
      return formatVersion({ major: parsed.major + 1, minor: 0, patch: 0, preTag: null, preNumber: null });
    case 'minor':
      return formatVersion({ major: parsed.major, minor: parsed.minor + 1, patch: 0, preTag: null, preNumber: null });
    case 'patch':
    ...
    case 'premajor':
      return formatVersion({ major: parsed.major + 1, minor: 0, patch: 0, preTag: preIdentifier, preNumber: 0 });
    case 'preminor':
      return formatVersion({ major: parsed.major, minor: parsed.minor + 1, patch: 0, preTag: preIdentifier, preNumber: 0 });
    case 'prepatch':
      return formatVersion({ major: parsed.major, minor: parsed.minor, patch: parsed.patch + 1, preTag: preIdentifier, preNumber: 0 });
    case 'prerelease':
...
const newVersion = bumpTypes.includes(version) ? bumpVersion(currentVersion, version, preid) : version;
