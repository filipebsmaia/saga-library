import { SagaStateDto, SagaEventDto, SagaMetricsDto, SagaStatus, EventHint } from '@/lib/types/saga';
...
export function generateSagaState(overrides: Partial<SagaStateDto> = {}): SagaStateDto {
  ...
  const updatedAt = overrides.updatedAt ?? new Date(new Date(startedAt).getTime() + Math.random() * 600_000).toISOString();
  const status = overrides.status ?? SagaStatus.RUNNING;
  const endedAt = status === SagaStatus.COMPLETED
    ? (overrides.endedAt ?? updatedAt)
    : null;
...
export function generateSagaEvent(
  ...
  const publishedAt = new Date(baseTime.getTime() + index * (2000 + Math.random() * 5000));
...
export function generateSagaMetrics(
  ...
  return {
    ...
    forkCount,
    childSagaCount,
    isStuck: saga.status !== SagaStatus.COMPLETED && (now - new Date(saga.updatedAt).getTime()) > 300_000,
  };
}
