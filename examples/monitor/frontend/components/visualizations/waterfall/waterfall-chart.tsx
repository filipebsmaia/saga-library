...
export function WaterfallChart({ tracks, spans }: WaterfallChartProps) {
  ...
  const dragStartRef = useRef<{ x: number; range: { start: number; end: number } } | null>(null);
  ...
  const canvasHeight = HEADER_HEIGHT + tracks.length * (TRACK_HEIGHT + TRACK_GAP) + PADDING * 2;
  ...
  useEffect(() => {
    ...
    for (const track of tracks) {
      ...
      const y = HEADER_HEIGHT + PADDING + idx * (TRACK_HEIGHT + TRACK_GAP) + TRACK_HEIGHT / 2 + 4;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (dragStartRef.current && e.buttons === 1) {
        ...
        const msPerPx = (dragStartRef.current.range.end - dragStartRef.current.range.start) / chartWidth;
        const deltaMs = -dx * msPerPx;

        const newStart = Math.max(0, dragStartRef.current.range.start + deltaMs);
        const rangeSize = dragStartRef.current.range.end - dragStartRef.current.range.start;
        const newEnd = Math.min(newStart + rangeSize, totalDuration * 1.1);

        setViewRange({ start: newEnd - rangeSize < 0 ? 0 : newStart, end: newEnd });
        setHoveredSpan(null);
        return;
      }
      ...
      for (const span of spans) {
        const idx = trackIndexMap.get(span.trackId);
        if (idx === undefined) continue;
        const spanY = HEADER_HEIGHT + PADDING + idx * (TRACK_HEIGHT + TRACK_GAP);
        if (y >= spanY && y <= spanY + TRACK_HEIGHT) {
          if (mouseMs >= span.startMs && mouseMs <= span.startMs + span.durationMs) {
            found = span;
            break;
          }
        }
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      setViewRange((prev) => {
        ...
        const newStart = Math.max(0, r.start + ratio * (currentRange - newRange));
    ...
    <div className={styles.container} ref={containerRef}>
      ...
      <div className={styles.canvasWrapper}>
        <canvas
          ...
          style={{ cursor: isDragging ? 'grabbing' : viewRange ? 'grab' : 'crosshair' }}
      ...
      {hoveredSpan && (
        <div
          ...
          style={{
            left: Math.min(tooltipPos.x + 12, (containerRef.current?.clientWidth ?? 800) - 300),
            top: (() => {
              const tooltipH = tooltipRef.current?.offsetHeight ?? 200;
              const wouldOverflow = tooltipPos.y + tooltipH + 12 > window.innerHeight;
              return wouldOverflow ? tooltipPos.y - tooltipH - 12 : tooltipPos.y + 12;
            })(),
          }}
        >
          <div className={styles.tooltipTitle}>{hoveredSpan.label}</div>
          {hoveredSpan.stepDescription && (
            <div className={styles.tooltipDesc}>{hoveredSpan.stepDescription}</div>
          )}
          <div className={styles.tooltipRow}>
            <span>Saga</span>
            <span>{hoveredSpan.sagaName ?? hoveredSpan.sagaId.slice(0, 8)}</span>
          </div>
          <div className={styles.tooltipRow}>
            <span>Duration</span>
            <span>{formatDuration(hoveredSpan.durationMs)} ({hoveredSpan.percentage.toFixed(1)}%)</span>
          </div>
          {hoveredSpan.hint && (
            <div className={styles.tooltipRow}>
              <span>Type</span>
          ...
          <div className={styles.tooltipRow}>
            <span>Status</span>
            <span>
              {hoveredSpan.statusBefore && hoveredSpan.statusBefore !== hoveredSpan.status
      ...
      <div style={{ padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <Skeleton variant="block" width={`${60 + Math.random() * 40}%`} height="24px" />
          </div>
        ))}
      </div>
    </div>
