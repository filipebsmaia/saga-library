...
import {
  SagaFilters,
  SagaFilterValues,
} from "@/components/dashboard/saga-filters/saga-filters";
import {
  SagaTable,
  SagaTableLoading,
  SagaTableEmpty,
...
export function LiveSagaTable({ initialData }: LiveSagaTableProps) {
  ...
  const { recentlyUpdatedIds, newSagasCount, resetNewCount } =
  ...
  const queryParams = useMemo(
    () => ({
      ...
      activeOnly:
        filters.quickFilters?.activeOnly ||
        filters.quickFilters?.incidentMode ||
        undefined,
  ...
  const filteredSagas = useMemo(() => {
    ...
    if (quickFilters.recentOnly) {
      result = result.filter(
        (saga) => now - new Date(saga.updatedAt).getTime() < 60_000,
      );
    }
      ...
      result = [...result].sort((a, b) => {
        const priority = (saga: SagaStateDto) => {
          const isStuck =
            ...
            now - new Date(saga.updatedAt).getTime() > STUCK_THRESHOLD;
          if (isStuck) {
            return 0;
          }
    ...
    <>
      ...
      {newSagasCount > 0 && (
        <button className={styles.refreshBanner} onClick={handleRefresh}>
          {newSagasCount} new saga{newSagasCount !== 1 ? "s" : ""} — Click to
          refresh
        </button>
      )}
