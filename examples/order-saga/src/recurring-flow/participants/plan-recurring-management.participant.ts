...
export class RFPlanRecurringManagementParticipant extends SagaParticipantBase {
  ...
  private readonly logger = new Logger(RFPlanRecurringManagementParticipant.name);
  ...
  async handlePlanOrderCompleted(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    await emit({
      ...
      stepDescription: 'plan-recurring-management marca recorrência como COMPLETED',
    ...
    const scheduledTo = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
  ...
  async handlePlanOrderPaymentFailed(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    if (record.totalAttempts >= record.maxAttempts) {
      ...
      await emit({
        ...
        stepDescription: 'plan-recurring-management marca recorrência como FAILED (max tentativas)',
      ...
      await emit({
        ...
        stepDescription: 'plan-recurring-management marca recorrência como CAPTURE_FAILED',
        payload: { recurringId, planId, customerId, reason },
        hint: 'final',
      });
    }
