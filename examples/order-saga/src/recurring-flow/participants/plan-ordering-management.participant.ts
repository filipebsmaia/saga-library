...
export class RFPlanOrderingManagementParticipant extends SagaParticipantBase {
  ...
  private readonly logger = new Logger(RFPlanOrderingManagementParticipant.name);
  ...
  async handlePlanOrderRequested(event: IncomingEvent, emit: Emit): Promise<void> {
    const { recurringId, planId, customerId, amount, cycle, orderType, simulatePaymentFailure } =
      event.payload as {
        recurringId: string;
        planId: string;
        customerId: string;
    ...
    this.logger.log(`PlanOrder ${planOrderId} created for recurring ${recurringId}`);
  ...
  async handleOrderPaymentFailed(event: IncomingEvent, emit: Emit): Promise<void> {
    const { planOrderId, recurringId, planId, customerId, reason } = event.payload as {
      planOrderId: string;
      recurringId: string;
      planId: string;
      customerId: string;
    ...
    await emit({
      ...
      stepDescription: 'plan-ordering-management atualiza ordem como PAYMENT_FAILED',
