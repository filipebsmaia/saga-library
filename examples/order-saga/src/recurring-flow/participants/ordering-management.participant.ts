...
export class RFOrderingManagementParticipant extends SagaParticipantBase {
  ...
  async handlePlanOrderCreated(event: IncomingEvent, emit: Emit): Promise<void> {
    const { planOrderId, recurringId, planId, customerId, amount, cycle, simulatePaymentFailure } =
      event.payload as {
        planOrderId: string;
        recurringId: string;
        planId: string;
  ...
  async handlePaymentSuccess(event: IncomingEvent, emit: Emit): Promise<void> {
    const { orderId, planOrderId, recurringId, planId, customerId, amount, cycle, paymentId } =
      event.payload as {
        orderId: string;
        planOrderId: string;
        recurringId: string;
    ...
    await emit({
      ...
      payload: { orderId, planOrderId, recurringId, planId, customerId, amount, cycle },
    });
  }

  @SagaHandler('rf.payment.created.failed')
  async handlePaymentFailed(event: IncomingEvent, emit: Emit): Promise<void> {
    const { orderId, planOrderId, recurringId, planId, customerId, reason } = event.payload as {
      orderId: string;
      planOrderId: string;
      recurringId: string;
      planId: string;
    ...
    await emit({
      ...
      payload: { orderId, planOrderId, recurringId, planId, customerId, reason },
      hint: 'compensation',
    });
  }
}
