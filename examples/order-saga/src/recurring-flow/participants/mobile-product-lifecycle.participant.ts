...
export class RFMobileProductLifecycleParticipant extends SagaParticipantBase {
  ...
  private readonly logger = new Logger(RFMobileProductLifecycleParticipant.name);
  ...
  async handleRecurringCreated(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    await emit({
      ...
      stepDescription: 'mobile-product-lifecycle cria produto móvel como PENDING',
      payload: { productId, customerId, planId, msisdn },
    });
  }

  @SagaHandler('rf.provision.completed')
  async handleProvisionCompleted(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(
      `Product ${productId} ACTIVE (provision: ${provisionId})`,
    );
