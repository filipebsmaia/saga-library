...
export class PlanOrderingManagementParticipant extends SagaParticipantBase {
  ...
  async handlePlanOrderRequested(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(`Bridging plan order for recurring ${recurringId} to ordering domain`);

    await emit({
      eventType: 'order.requested',
      stepName: 'bridge-to-ordering',
      payload: { recurringId, planId, customerId, amount, cycle, simulatePaymentFailure, simulateTransient },
    });
  }
  ...
  async handleOrderCompleted(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(`Order ${orderId} completed — bridging back to plan domain`);
  ...
  async handleOrderFailed(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.warn(`Order ${orderId} failed — bridging failure back to plan domain`);
