...
export class UpgradeEligibilityParticipant extends SagaParticipantBase {
  ...
  async handleUpgradeRequested(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(`Validating eligibility for upgrade ${upgradeId}: ${currentPlan} → ${targetPlan}`);
    ...
    await emit({
      ...
      payload: { upgradeId, customerId, currentPlan, targetPlan, simulateFailure },
    });
  }
}
