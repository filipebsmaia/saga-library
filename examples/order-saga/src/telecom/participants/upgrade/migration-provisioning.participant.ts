...
export class MigrationProvisioningParticipant extends SagaParticipantBase {
  ...
  async handleMigrationStarted(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(`Provisioning new plan resources for upgrade ${upgradeId} (ref: ${provisioningId})`);

    await emit({
      eventType: 'migration.provisioned',
      stepName: 'Provision New Plan',
      stepDescription: 'New plan resources provisioned',
      payload: { upgradeId, customerId, currentPlan, targetPlan, provisioningId, simulateFailure },
    });
  }
}
