...
export class LineActivationParticipant extends SagaParticipantBase {
  ...
  async handleLineActivationRequested(event: IncomingEvent, emit: Emit): Promise<void> {
    ...
    this.logger.log(`Line ${lineIndex} (${lineNumber}) activated for bulk ${bulkId}`);

    await emit({
      eventType: 'line-activation.completed',
      stepName: 'complete-line-activation',
      payload: { bulkId, lineIndex, lineNumber, activatedAt: new Date().toISOString() },
    });
  }
}
