...
export class TelecomController {
  ...
  async triggerRecurring(
    ...
    }, { sagaName: 'Cobrança Recorrente', sagaDescription: 'Ciclo mensal de cobrança do plano móvel' });
  ...
  async triggerSimSwap() {
    ...
    }, { sagaName: 'sim-swap', sagaDescription: 'SIM card swap with portability check' });
  ...
  async triggerBulkActivation(@Query('lines') linesParam?: string) {
    ...
    }, { sagaName: 'bulk-activation', sagaDescription: 'Bulk SIM line activation batch' });

    this.bulkActivationStore.create(bulkId, sagaId, { totalLines: lines });

    this.logger.log(`Bulk activation saga ${sagaId} started for ${lines} lines`);
  ...
  async triggerUpgrade(@Query('fail') fail?: string) {
    ...
    }, { sagaName: 'Upgrade de Plano', sagaDescription: `Upgrade ${currentPlan} → ${targetPlan}` });

    this.upgradeStore.create(upgradeId, sagaId, { customerId, currentPlan, targetPlan });

    this.logger.log(`Upgrade saga ${sagaId} started for upgrade ${upgradeId}`);

    return { upgradeId, sagaId, customerId, currentPlan, targetPlan, status: 'REQUESTED' };
  }
